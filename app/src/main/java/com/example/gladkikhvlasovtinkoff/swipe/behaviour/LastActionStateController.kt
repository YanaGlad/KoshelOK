    ...
    fun onTranslate(mainView: View, actionView: View, actionSize: Size, dx: Int, index: Int) {
        when {
            isFullyOpened(mainView, actionSize) && !isOpeningOrOpened() -> {
                ...
                animation = createAnimation(delegate.createOpeningAnimation(mainView, actionView, actionSize),
                    State.OPENED
                )
                animation?.start()
            }
            !isFullyOpened(mainView, actionSize) && !isClosingOrClosed() -> {
                ...
                animation = createAnimation(delegate.createClosingAnimation(mainView, actionView, actionSize),
                    State.CLOSED
                )
                animation?.start()
            }
